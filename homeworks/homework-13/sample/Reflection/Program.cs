...
class Program
{
    static void Main(string[] args)
    {
        ...
        try
        {
            ...
            MethodInfo getMethod = stringLengthField.GetGetMethod();
        ...
        Func<TestClass, string> getDelegate = (Func<TestClass, string>)
                Delegate.CreateDelegate(typeof(Func<TestClass, string>), property.GetGetMethod(nonPublic: true));
        ...
        // Свойство (stock.Price)

        // PropertyInfo (pi.GetValue())

        // Cached PropertyInfo

        // Делегат ((Func<>) Delegate.CreateDelegate())

        // Динамический делегат (Delegate.CreateDelegate())
        ...
        // Reflection.Emit -> генрирует/работает с промежуточным кодом (MSIL)
